...
export default function Home() {
  ...
  const downloadYoutubeVid = async ({ passedURL }: { passedURL: string }) => {
    try {
      ...
      const res = await fetch(`/api/download-youtube?passedURL=${encodeURIComponent(passedURL)}`);
      ...
      if (!data.formats || (!data.formats.video?.length && !data.formats.audio?.length)) {
  ...
  const handleVideoDownload = () => {
    ...
    const selectedFormat = videoData.formats.video.find(
      (format: any) => format.quality && format.quality.includes(qualityMap[selectedVideoQuality])
    ...
    a.download = `${videoData.title || 'youtube-video'}.${selectedFormat.container || 'mp4'}`;
  ...
  const handleAudioDownload = () => {
    ...
    const a = document.createElement('a');
    a.href = selectedFormat.url;
    a.download = `${videoData.title || 'youtube-audio'}.${selectedFormat.container || 'm4a'}`;
    a.target = "_blank"; // Open in new tab
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    ...
    <div className="w-full min-h-screen bg-background flex flex-col">
      ...
      <main className="flex-1 container mx-auto py-12 px-4 md:px-6">
        <div className="max-w-3xl mx-auto space-y-8">
          ...
          {platform && platform !== "youtube" && platform !== "facebook" && platform !== "twitter" && (
            <Card className="border-orange-500/20">
              <CardHeader>
                <div className="flex items-center gap-3">
